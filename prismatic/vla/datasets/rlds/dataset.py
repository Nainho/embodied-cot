...
def make_dataset_from_rlds(
    ...
    *,
    ...
    def make_tf_dict(raw_dict):
        print("📦 Building Libero-style reasoning tf.lookup.StaticHashTable...")

        keys = []
        values = []
        has_reasoning = [0, 0]  # [없음, 있음]
        ...
        def reasoning_dict_to_str(d):
            ...
            for tag in tags:
                ...
                # PLAN: dict → "0. step1; 1. step2"
                if isinstance(val, dict) and tag == "PLAN:":
                    val = "; ".join([f"{k}. {v}" for k, v in sorted(val.items(), key=lambda x: int(x[0]))])

                # VISIBLE OBJECTS: dict of name → [[x0,y0], [x1,y1]]
                elif isinstance(val, dict) and tag == "VISIBLE OBJECTS:":
                    def flatten_box(box):
                        return [coord for pt in box for coord in pt]
                    val = ", ".join([
                        f"{name} {flatten_box_str(flatten_box(box))}"
                        for name, box in val.items()
                    ])
        ...
        def flatten_box_str(lst):
            return "[" + ", ".join(map(str, lst)) + "]"

        for file_name in raw_dict:
            for episode_id in raw_dict[file_name]:
                for step_id in raw_dict[file_name][episode_id]:
                    reasoning_dict = raw_dict[file_name][episode_id][step_id]

                    if reasoning_dict is None or not isinstance(reasoning_dict, dict):
                        has_reasoning[0] += 1
                        continue

                    has_reasoning[1] += 1

                    key = f"{file_name}_{episode_id}_{step_id}"
                    value = reasoning_dict_to_str(reasoning_dict)

                    keys.append(key)
                    values.append(value)

        print(f"✅ Built {len(keys)} reasoning entries.")
        print(f"📊 Reasoning presence stats: [missing: {has_reasoning[0]}, present: {has_reasoning[1]}]")
        print(f"🔑 Example key: {keys[0]}")
        print(f"📘 Example reasoning:\n{values[0][:300]}{'...' if len(values[0]) > 300 else ''}")
